...
func TestClientRateLaptop(t *testing.T) {
	...
	laptopStore := NewInMemoryLaptopStore()
	ratingStore := NewInMemeoryRatingStore()
	...
	for idx := 0; ; idx++ {
		log.Print("server: receiving")
		res, err := stream.Recv()
		log.Print("server: received")
		if err == io.EOF {
...
func startTestLaptopServer(
	...
	listener, err := net.Listen("tcp", ":0") // random available port (":0")
	require.NoError(t, err)
